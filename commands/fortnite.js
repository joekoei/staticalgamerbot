...
const fkey = require("../keys.json")
const fortnite = new Client(fkey.token)
module.exports.run = async (bot, message, args) => {
  //this is where the actual code for the command goes
  await message.delete();
  if (message.author.id !== '439820800292290570') return;

  const filter = m => m.author.id === message.author.id;
  message.reply("Please choose a username... Will expire in 10 seconds...").then(q => q.delete(15000))
  message.channel.awaitMessages(filter, {
    ...
    fortnite.user(username, 'psn').then(data => {
